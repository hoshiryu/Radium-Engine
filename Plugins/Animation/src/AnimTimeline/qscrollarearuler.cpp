...
void QScrollAreaRuler::keyPressEvent( QKeyEvent* event ) {
    ...
    case Qt::Key_I:
        if ( shiftDown ) { emit removeKeyPose(); }
        else
        {
            emit addKeyPose();
...
void QScrollAreaRuler::wheelEvent( QWheelEvent* event ) {
    ...
    else if ( ctrlDown )
    {
        horizontalScrollBar()->setValue(
            static_cast<int>( horizontalScrollBar()->value() + ry * SLIDE_SPEED ) );
    }
    // zoom in/out
    else
    {
        int newRulerWidth = static_cast<int>( ruler->minimumWidth() +
                                              ry * ZOOM_SPEED * ruler->minimumWidth() / width() );
        if ( newRulerWidth <= width() - 2 )
        {
            if ( ruler->minimumWidth() == width() - 2 ) { return; }
            else
